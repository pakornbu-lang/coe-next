...
export default async function Page({searchParams}:{searchParams:Promise<{status?:string;reviewer?:string}>}){
 ...
 <form className="workflow-inline-form"><label>สถานะ<select name="status" defaultValue={f.status??""}><option value="">ทั้งหมด</option><option value="assigned">ค้างประเมิน</option><option value="completed">ส่งแล้ว</option><option value="revoked">ถอนงาน</option><option value="overdue">เกินกำหนด</option></select></label><label>กรรมการ<select name="reviewer" defaultValue={f.reviewer??""}><option value="">ทุกคน</option>{p.data.map(x=><option key={x.id} value={x.id}>{x.full_name} · ค้าง {pending.filter(r=>r.reviewer_id===x.id).length}</option>)}</select></label><button className="btn">กรอง</button></form></section>
