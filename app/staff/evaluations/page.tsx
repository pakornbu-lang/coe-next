...
// คะแนนในรายงานเป็นผลรวม/ค่าเฉลี่ยเพื่อแสดง ไม่ใช่คำสั่งอนุมัติทุนอัตโนมัติ
 ...
 <div style={{overflowX:"auto"}}><table className="review-score-table"><thead><tr><th>กรรมการ</th>{criteria.map(k=><th key={k.id}>{k.label} / {k.max_score}</th>)}<th>รวม</th><th>ข้อเสนอ / ความเห็น</th></tr></thead><tbody>{assignments.map(x=>{const ev=e.data.find(e=>e.assignment_id===x.id&&e.submitted_at);const scores=(ev?.scores??[]) as {criterion_id:string;score:number}[];return <tr key={x.id}><td>{p.data.find(p=>p.id===x.reviewer_id)?.full_name??"กรรมการ"}</td>{criteria.map(k=><td key={k.id}>{scores.find(s=>s.criterion_id===k.id)?.score??"—"}</td>)}<td>{ev?.total_score??"ยังไม่ส่ง"}</td><td>{ev?({approve:"เสนออนุมัติ",reserve:"เสนอสำรอง",reject:"ไม่เสนออนุมัติ"}[ev.recommendation as "approve"]):"รอผล"} {ev?.comment}</td></tr>})}</tbody><tfoot><tr><th>เฉลี่ยรายเกณฑ์</th>{criteria.map(k=>{const values=submitted.flatMap(e=>(e.scores as {criterion_id:string;score:number}[]).filter(s=>s.criterion_id===k.id).map(s=>Number(s.score)));return <td key={k.id}>{values.length?(values.reduce((a,b)=>a+b,0)/values.length).toFixed(2):"—"}</td>;})}<td colSpan={2}>เฉพาะคะแนนที่ส่งแล้ว</td></tr></tfoot></table></div><Link href={"/staff/evaluations/"+app.id}>เปิดหน้าสรุปและตัดสินผล</Link></section>})}</div>;
}
