...
export default async function ApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!detail) notFound();
  const { application, scholarship, documents, paymentAccount, disbursement, history, interview, appeal } = detail;
  const statusLabel = (status: string) => applicationStatusLabels[status as ApplicationStatus] ?? status;
  // eslint-disable-next-line react-hooks/purity
  const isAppealDeadlineValid = scholarship.appeal_deadline ? Date.now() <= new Date(scholarship.appeal_deadline).getTime() : false;
  const canAppeal = !appeal && ["reserve", "rejected"].includes(application.status) && Boolean(scholarship.results_published_at) && Boolean(scholarship.appeal_deadline) && isAppealDeadlineValid;
  ...
  return <div className="workflow-stack">
    ...
    {["draft", "revision_requested"].includes(application.status) && <section className="panel workflow-info">คุณยังแก้ไขใบสมัครได้ <Link href={`/apply?scholarshipId=${scholarship.id}&applicationId=${application.id}`}>กลับไปแก้ไขใบสมัคร</Link></section>}
