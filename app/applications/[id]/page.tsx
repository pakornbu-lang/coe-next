...
export default async function ApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="workflow-stack">
    ...
    <section className="panel"><h2>เอกสารประกอบ</h2><div className="workflow-row-list">{documents.map((document) => <Link href={`/documents/${document.id}`} key={document.id}><span><strong>{document.requirement?.label ?? "เอกสาร"}</strong><small>{document.file_name}{document.feedback ? ` · ${document.feedback}` : ""}</small></span><span className={`document-state ${document.status}`}>{document.status === "verified" ? "ผ่านการตรวจ" : document.status === "revision_required" ? "ขอแก้ไข" : "รอตรวจ"}</span></Link>)}</div></section>
