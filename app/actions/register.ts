...
export async function registerStudent(_previous:RegisterState,form:FormData):Promise<RegisterState>{
 ...
 if(!firstName||!lastName||firstName.length>96||lastName.length>96)
   return {error:"กรุณากรอกชื่อและนามสกุลให้ครบ โดยแต่ละช่องไม่เกิน 96 ตัวอักษร",success:""};
 ...
 if(fullName.length>200||!/^[0-9]{8,12}$/.test(studentId)||!/^([^\s@]+)@(mail\.)?wu\.ac\.th$/.test(email)||email.length>254)
