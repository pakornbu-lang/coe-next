...
const failedRegistration =
  "สมัครสมาชิกไม่สำเร็จ กรุณาตรวจสอบข้อมูล หากเคยสมัครแล้วให้เข้าสู่ระบบหรือติดต่อผู้ดูแล";
...
async function reserveRegistrationAttempt(
  ...
  if (error) {
    throw new Error("Registration rate limit unavailable");
  }
...
export async function registerStudent(
  ...
  if (
    !["นาย", "นางสาว", "นาง"].includes(prefix) ||
    !firstName ||
    ...
    !/^[^\s@]+@mail\.wu\.ac\.th$/.test(email) ||
    email.length > 254
  ) {
    return {
      error:
        "กรุณากรอกชื่อ นามสกุล รหัสนักศึกษา 8–12 หลัก และอีเมล @mail.wu.ac.th ให้ถูกต้อง",
      success: "",
    };
  }
  ...
  if (
    password.length < 8 ||
    password.length > 128 ||
    password !== String(form.get("confirm_password") ?? "")
  ) {
    return {
      error:
        "รหัสผ่านต้องยาว 8–128 ตัวอักษร และทั้งสองช่องต้องตรงกัน",
      success: "",
    };
  }
  ...
  try {
    ...
    if (signIn.error || !signIn.data.user) {
      return {
        error: "",
        success:
          "สร้างบัญชีแล้ว กรุณาเข้าสู่ระบบด้วยอีเมลและรหัสผ่านที่เพิ่งตั้ง",
      };
    }
    ...
    if (!viewer || viewer.role !== "student") {
      ...
      return {
        error:
          "สร้างบัญชีแล้ว แต่ยังเปิดหน้าใช้งานไม่ได้ กรุณาติดต่อผู้ดูแล",
        success: "",
      };
    }
  } catch {
