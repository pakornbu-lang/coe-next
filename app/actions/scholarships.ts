...
export async function saveScholarship(
  ...
  /*
   * ใช้ภาพปกเดิมเป็นค่าเริ่มต้น
   * ถ้าไม่ได้เลือกภาพใหม่ ภาพเดิมจะยังอยู่
   */
  let coverPath =
    existingCoverPath;
  ...
  /*
   * เพิ่มเฉพาะส่วนนี้:
   * ภาพจะถูกลบต่อเมื่อผู้ใช้เลือก "ลบภาพปกเดิม"
   */
  const removeCover =
    value(form, "remove_cover") ===
    "true";
  ...
  if (error || !data) {
    /*
     * ถ้าเพิ่งอัปโหลดภาพใหม่แล้วบันทึกข้อมูลไม่สำเร็จ
     * ลบเฉพาะภาพใหม่ที่เพิ่งอัปโหลด
     */
    if (
      coverPath &&
      coverPath !==
