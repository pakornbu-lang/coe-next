...
export async function manageReview(_:OperationState,f:FormData):Promise<OperationState>{
 ...
 if(error)return {error:error.code==="40001"?"ข้อมูลเปลี่ยนแล้ว กรุณารีเฟรช":error.code==="23505"?"กรรมการนี้มีงานในใบสมัครเดียวกันแล้ว":"ตรวจสอบกำหนดส่ง เหตุผล และสถานะงาน",success:""};
...
export async function saveInterview(_:OperationState,f:FormData):Promise<OperationState>{
 ...
 if(error)return {error:error.code==="23P01"?"เวลาซ้อนกับนัดของกรรมการ นักศึกษา หรือสถานที่":error.code==="40001"?"นัดถูกแก้ไขแล้ว กรุณารีเฟรช":"ตรวจสอบเวลา กรรมการ และผลสัมภาษณ์",success:""};
