  ...
  return <div className="workflow-stack">
    ...
    <form action={action} className="workflow-form">
      ...
      <section className="panel"><h2>ข้อมูลการศึกษาและการติดต่อ</h2><div className="workflow-grid">
        ...
        <label>ระดับการศึกษา<input name="education_level" maxLength={50} defaultValue={data.education_level ?? details.education_level ?? "ปริญญาตรี"}/></label>
        <label>ชั้นปี<input name="study_year" type="number" min="1" max="8" defaultValue={data.study_year ?? details.study_year ?? ""}/></label>
        ...
        <label>เบอร์โทรศัพท์<input name="phone" type="tel" maxLength={25} defaultValue={data.phone ?? profile?.phone ?? ""}/></label>
        <label className="workflow-wide">ที่อยู่ติดต่อ<textarea name="address" maxLength={500} rows={3} defaultValue={data.address ?? details.address ?? ""}/></label>
        <label>รายได้ครอบครัวต่อปี (บาท)<input name="income" inputMode="numeric" maxLength={30} defaultValue={data.income ?? ""}/></label>
        <label>จำนวนสมาชิกในครอบครัว<input name="family_members" inputMode="numeric" maxLength={3} defaultValue={data.family_members ?? ""}/></label>
      ...
      <section className="panel"><h2>บัญชีรับเงิน</h2><p className="workflow-muted">ข้อมูลส่วนนี้เห็นได้เฉพาะนักศึกษาและเจ้าหน้าที่ผู้รับผิดชอบการจ่ายทุน</p><div className="workflow-grid"><label>ธนาคาร *<input name="bank_name" required maxLength={150} defaultValue={paymentAccount?.bank_name ?? ""}/></label><label>ชื่อบัญชี *<input name="account_holder" required maxLength={200} defaultValue={paymentAccount?.account_holder ?? ""}/></label><label>เลขบัญชี *<input name="account_number" required inputMode="numeric" maxLength={30} defaultValue={paymentAccount?.account_number ?? ""}/></label></div></section>
      <div className="workflow-actions"><Link className="btn secondary" href={`/scholarships/${scholarship.id}`}>กลับรายละเอียดทุน</Link><button className="btn secondary" name="mode" value="draft" disabled={pending}>{pending ? "กำลังบันทึก…" : "บันทึกร่าง"}</button><button className="btn" name="mode" value="submit" disabled={pending}>{pending ? "กำลังส่ง…" : "ส่งใบสมัคร"}</button></div>
      {state.error && <p role="alert" className="workflow-error">{state.error}</p>}{state.success && <p role="status" className="workflow-success">{state.success}</p>}
    </form>
