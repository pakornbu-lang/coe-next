...
Migration `20260924171000_enforce_minimum_gpa.sql` ตรวจ GPA เทียบขั้นต่ำเมื่อส่งใบสมัคร รวมการส่งแก้ไข คะแนนเท่าขั้นต่ำผ่านได้ ร่างยังบันทึกได้แม้คะแนนต่ำกว่าเกณฑ์ ทุนที่ไม่กำหนดขั้นต่ำไม่มีข้อจำกัด GPA เพิ่มเติมจาก migration นี้ (RPC ยังคงตรวจรูปแบบข้อมูลตามเดิม) การตรวจใช้ข้อมูลที่กรอกในใบสมัคร ไม่ใช่การยืนยันผลการเรียนจากมหาวิทยาลัย

ชุดทดสอบ `supabase/tests/minimum_gpa.sql` ใช้ฐานข้อมูลทดสอบหลัง migrations และ rollback fixtures ทั้งหมด
